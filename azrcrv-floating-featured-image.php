...
add_action('admin_init', 'azrcrv_create_plugin_menu_ffi');
...
add_action('plugins_loaded', 'azrcrv_ffi_load_languages');

// add filters
add_filter('plugin_action_links', 'azrcrv_ffi_add_plugin_action_link', 10, 2);
add_action('the_posts', 'azrcrv_ffi_check_for_shortcode');
add_filter('codepotent_update_manager_image_path', 'azrcrv_ffi_custom_image_path');
add_filter('codepotent_update_manager_image_url', 'azrcrv_ffi_custom_image_url');
 ...
 *
 */
function azrcrv_ffi_load_languages() {
    $plugin_rel_path = basename(dirname(__FILE__)).'/languages';
    load_plugin_textdomain('floating-featured-image', false, $plugin_rel_path);
}
 ...
 *
 */
function azrcrv_ffi_custom_image_path($path){
    if (strpos($path, 'azrcrv-floating-featured-image') !== false){
 ...
 *
 */
function azrcrv_ffi_custom_image_url($url){
    if (strpos($url, 'azrcrv-floating-featured-image') !== false){
 ...
 *
 */
function azrcrv_ffi_get_option($option_name){
...
function azrcrv_ffi_add_plugin_action_link($links, $file){
	...
	if ($file == $this_plugin){
		$settings_link = '<a href="'.admin_url('admin.php?page=azrcrv-ffi').'"><img src="'.plugins_url('/pluginmenu/images/Favicon-16x16.png', __FILE__).'" style="padding-top: 2px; margin-right: -5px; height: 16px; width: 16px;" alt="azurecurve" />'.esc_html__('Settings' ,'floating-featured-image').'</a>';
...
function azrcrv_ffi_display_options() {
	...
	$options = azrcrv_ffi_get_option( 'azrcrv-ffi' );
	?>
